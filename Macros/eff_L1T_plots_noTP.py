...
in_files['ZB_355207_355383_noTP'] = ROOT.TFile( 'plots/L1T_eff_Pt22_2022B_BX0_uGMT_HLT_UNP_ZB_355207_355383_no_TP.root' )
...
legText_summary ={}
legText_summary['uGMTSingleMu7']    = ['0 #leq |#eta^{#mu, offline}| #leq 2.4']
legText_summary['BMTFSingleMu7']    = ['0 #leq |#eta^{#mu, offline}| #leq 0.83']
legText_summary['OMTFSingleMu7']   = ['0.83 #leq |#eta^{#mu, offline}| #leq 1.24']
legText_summary['EMTFSingleMu7']   = ['1.24 #leq |#eta^{#mu, offline}| #leq 2.4']
...
for TF in TFs:
  for in_file_key in in_files.keys():
    for WP in WPs.keys():
      h_pt[in_file_key+TF+WP] = ROOT.TH1F()
      h_pt_2[in_file_key+TF+WP] = ROOT.TH1F()
      h_eta[in_file_key+TF+WP] = ROOT.TH1F()
      h_phi[in_file_key+TF+WP] = ROOT.TH1F()
...
for TF in TFs:
  for in_file_key in in_files.keys():
    for WP in WPs.keys():
      print(TF + WP)
      h_pt[in_file_key+TF+WP] = in_files[in_file_key].Get('h_pt_%s_%s' % (TF, WP)).Clone()
      h_pt_2[in_file_key+TF+WP] = in_files[in_file_key].Get('h_pt_2_%s_%s' % (TF, WP)).Clone()
      h_eta[in_file_key+TF+WP] = in_files[in_file_key].Get('h_eta_%s_%s' % (TF, WP)).Clone()
      h_phi[in_file_key+TF+WP] = in_files[in_file_key].Get('h_phi_%s_%s' % (TF, WP)).Clone()

      h_pt_trg[in_file_key+TF+WP] = in_files[in_file_key].Get('h_pt_trg_%s_%s' % (TF, WP)).Clone()
      h_pt_2_trg[in_file_key+TF+WP] = in_files[in_file_key].Get('h_pt_2_trg_%s_%s' % (TF, WP)).Clone()
      h_eta_trg[in_file_key+TF+WP] = in_files[in_file_key].Get('h_eta_trg_%s_%s' % (TF, WP)).Clone()
      h_phi_trg[in_file_key+TF+WP] = in_files[in_file_key].Get('h_phi_trg_%s_%s' % (TF, WP)).Clone()
...
for TF in TFs:
  for in_file_key in in_files.keys():
    for WP in WPs.keys():
      eff_pt[in_file_key+TF+WP] = ROOT.TGraphAsymmErrors(h_pt_trg[in_file_key+TF+WP],h_pt[in_file_key+TF+WP])
      eff_pt_2[in_file_key+TF+WP] = ROOT.TGraphAsymmErrors(h_pt_2_trg[in_file_key+TF+WP],h_pt_2[in_file_key+TF+WP])
      eff_eta[in_file_key+TF+WP] = ROOT.TGraphAsymmErrors(h_eta_trg[in_file_key+TF+WP],h_eta[in_file_key+TF+WP])
      eff_phi[in_file_key+TF+WP] = ROOT.TGraphAsymmErrors(h_phi_trg[in_file_key+TF+WP],h_phi[in_file_key+TF+WP])
...
for TF in TFs:
  ...
  for in_file_key in in_files.keys():
    ...
    for WP in WPs.keys():
      if WP == 'SingleMu7': eff_pt[in_file_key+TF+WP].Draw("AP")
